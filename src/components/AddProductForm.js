...
function AddProductForm ({ refreshProductList }) {
  ...
  const handleAddProductClick = async () => {
    try {
        ...
        const response = await fetch('http://localhost:3001/api/addUserProduct', {
            method: 'POST',
            headers: {
        ...
        if (response.ok) {
            ...
            if (errorData && errorData.error) {
                ...
                console.error('Wystąpił problem podczas dodawania produktu');
            }
        }
    } catch (error) {
        console.error('Błąd podczas żądania do API:', error);
